...
class TopologyRepScript(appionScript.AppionScript):
	...
	def runIMAGICmsa(self):
		...
		if self.params['msaproc'] > 1:
			f.write("mpirun -np %i "%(self.params['msaproc']))
			## for barcelona queue:
	...
	def start(self):
		...
		if self.params['cluster'] is not None and self.params['uploadonly'] is not True:
			## copy stuff to local directory
			actualrundir=self.params['rundir']
			self.params['rundir']=re.sub("^/appion","/home",self.params['rundir'])
			mkdircmd = "mkdir -p %s"%self.params['rundir']
			subprocess.Popen(mkdircmd,shell=True).wait()
			cpcmd = "cp %s.* %s/."%(os.path.splitext(self.params['localstack'])[0],self.params['rundir'])
			subprocess.Popen(cpcmd,shell=True).wait()
			self.params['localstack']=re.sub("^/appion","/home",self.params['localstack'])
		...
		## if cluster, move files to /appion
		if self.params['cluster'] is not None and self.params['uploadonly'] is not True:
			movecmd = "mv -f classes*.* iter* mrastack.* average* %s"%actualrundir
			subprocess.Popen(movecmd,shell=True).wait()
			## check that files were copied, then remove work directory
			finalcls = "classes%02i.img" % self.params['currentiter']
			if os.path.exists(os.path.join(actualrundir,finalcls)):
				apDisplay.printWarning("files copied, removing cluster directory")
				apFile.removeDir(self.params['rundir'])
			self.params['rundir'] = actualrundir
			# move back to starting directory
			os.chdir(self.params['rundir'])
